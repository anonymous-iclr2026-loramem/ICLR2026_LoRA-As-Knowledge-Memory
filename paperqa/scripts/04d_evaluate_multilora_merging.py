...
Experiments with various merging methods for LoRA adapters of N specified papers.
"""
...
# Add project root to sys.path (Adjust based on environment)
...
class PaperQAMergingEvaluator:
    ...
    def __init__(self, base_model_id: str, lora_adapters_dir: str, metrics: List[str], 
        ...
        Args:
            ...
            question_format: Question format style ('bracket' or 'natural').
            merging_params: Additional parameters for merging methods.
            routing_method: Routing method ('perfect' or 'rag').
            retrieval_top_k: Number of adapters to retrieve for RAG routing.
        """
        ...
        # Selectively load LoRA adapters corresponding to specified paper IDs
        ...
        # Merge all adapters only if using Perfect routing
    ...
    def _load_lora_adapters(self) -> List[Dict]:
        """Loads LoRA adapters and collects metadata."""
            ...
            if os.path.isdir(item_path):
                ...
                if os.path.exists(adapter_path):
                    ...
                    if paper_match:
                        ...
                        # Fallback: try to extract numbers from the full directory name
                        numbers = re.findall(r'\d+', item)
                        if numbers:
                            paper_id = int(numbers[0])
                        else:
                            paper_id = hash(item) % 10000
                        logging.warning(f"Failed to extract paper ID, using fallback ID: {item} -> {paper_id}")
    ...
    def _filter_adapters(self, all_adapters: List[Dict]) -> List[Dict]:
        """Filters adapters corresponding to the requested paper_indices."""
        ...
        if missing_ids:
            logging.warning(f"Could not find LoRA adapters for the following paper IDs: {sorted(list(missing_ids))}")
    ...
    def _setup_retriever(self):
        """Sets up the LoRA adapter retriever based on paper introductions."""
        # TODO: Update this path to your PaperQA dataset JSON file
        paperqa_data_path = "data/paper_dataset/paperQA.json"
        ...
        # Map paper IDs to introductions
        paper_introductions = {}
        ...
        # Collect introductions for all papers (for RAG routing)
        available_introductions = []
        introduction_idx_to_adapter = {}
        
        all_paper_ids = list(range(15))  # Assuming IDs 0-14
        ...
        for paper_id in all_paper_ids:
            if paper_id in paper_introductions:
                ...
                if adapter:
                    ...
                    # Create placeholder info for mapping purposes
                    introduction_idx_to_adapter[len(available_introductions) - 1] = {
                        'name': f'paperqa_multilora_paper{paper_id}_qa_bracket_r16',
                        'paper_id': paper_id,
                        'path': f'/path/to/paper{paper_id}/final',
                        'adapter_path': f'/path/to/paper{paper_id}/final/adapter_model.safetensors'
                    }
            else:
        ...
        def retrieve_adapters(query: str, top_k: int = None) -> List[Dict]:
            """Returns the most relevant LoRA adapters for a query."""
            if top_k is None:
    ...
    def _load_eval_data(self) -> List[dict]:
        """Loads evaluation data for specified papers."""
        # TODO: Update this path to your PaperQA evaluation dataset JSON file
    ...
    def _setup_all_adapters(self) -> PeftModel:
        """Pre-loads all selected LoRA adapters into the PeftModel."""
        if not self.selected_adapters:
            raise ValueError("No LoRA adapters to load.")
        
        logging.info(f"Starting load of all LoRA adapters: {len(self.selected_adapters)} items")
    ...
    def _setup_peft_model(self) -> PeftModel:
        """Merges all selected LoRA adapters into the PeftModel."""
        if not self.selected_adapters:
        ...
        # Clean up previous merged adapters
        ...
        try:
            logging.info(f"Merging adapters using method: '{self.merging_method}'...")
            ...
            logging.info(f"Merge complete. Active adapter: {merged_name}")
            return self.peft_model
            
        except Exception as e:
            logging.error(f"Adapter merging failed: {e}")
            logging.warning("Using the first adapter due to merge failure.")
            self.peft_model.set_adapter(self.selected_adapters[0]['name'])
            return self.peft_model
    
    def _generate_answer(self, question: str, paper_id: int, retrieved_adapters: List[Dict] = None) -> Tuple[str, str]:
        """Generates an answer based on the routing method."""
        if self.routing_method == "perfect":
    ...
    def _generate_answer_perfect(self, question: str, paper_id: int) -> Tuple[str, str]:
        """Perfect routing: Generate answer using the model with all adapters merged."""
    ...
    def _generate_answer_rag(self, question: str, paper_id: int, retrieved_adapters: List[Dict] = None) -> Tuple[str, str]:
        ...
        if not retrieved_adapters:
            logging.warning("No adapters retrieved. Using base model cannot generate specific answers.")
            formatted_question = f"[Paper ID: {paper_id}] {question}"
            predicted_answer = "Unable to generate answer due to missing relevant adapters."
            return formatted_question, predicted_answer
        
        # Setup PeftModel with retrieved adapters
    ...
    def _setup_peft_model_for_adapters(self, adapters: List[Dict]) -> PeftModel:
        """Sets up PeftModel with selected LoRA adapters (specifically for RAG routing)."""
        if not adapters:
            raise ValueError("No adapters to set up.")
        
        # Filter for adapters that are actually loaded
        available_adapters = []
        ...
        logging.info(f"Retrieved: {retrieved_info}")
        logging.info(f"Available: {available_info}")
        
        if not available_adapters:
            logging.warning("No retrieved adapters are loaded. Using the first selected adapter as fallback.")
        ...
        try:
            ...
            logging.error(f"Adapter merging failed: {e}")
            logging.warning("Using first adapter due to merge failure.")
...
"""
    ...
    def evaluate(self) -> Dict:
        ...
        for item in tqdm(eval_data, desc=f"Evaluating ({self.routing_method}-{self.merging_method})", unit="sample"):
            ...
            # Perform retrieval if using RAG routing
            ...
            try:
                ...
                logging.error(f"Failed to generate answer (Paper ID: {paper_id}): {e}")
                formatted_question = f"[Paper ID: {paper_id}] {question}"
...
def main():
    parser = argparse.ArgumentParser(description="PaperQA LoRA Merging Evaluation")
    ...
    parser.add_argument("--paper-indices", type=int, nargs='+', required=True, help="List of paper IDs for the experiment (e.g., 0 1 2 3)")
    parser.add_argument("--merging-method", type=str, choices=['linear', 'cat', 'ties'], required=True, help="LoRA merging method")
    parser.add_argument("--routing-method", type=str, choices=['perfect', 'rag'], default='perfect', help="Routing method (perfect: merge all, rag: retrieve relevant)")
    parser.add_argument("--retrieval-top-k", type=int, default=3, help="Number of adapters to retrieve in RAG routing (default: 3)")
    ...
    logging.info(f"  LoRA Adapters Dir: {args.lora_adapters_dir}")
    ...
    print(f"Number of Papers: {summary['num_lora_adapters']}")
    print(f"Evaluated Samples: {summary['num_evaluated_samples']}")
...
if __name__ == "__main__":
    # Example Usage:
    # Perfect routing (Merge all):
    # python evaluate_merging_methods.py \
    #   --base-model-id /path/to/base_model \
    #   --lora-adapters-dir /path/to/lora_outputs \
    #   --output-dir /path/to/evaluation_results \
    #   --paper-indices 0 2 4 8 \
    #   --merging-method linear \
    #   --routing-method perfect
    #
    # RAG routing (Retrieve relevant):
    # python evaluate_merging_methods.py \
    #   --base-model-id /path/to/base_model \
    #   --lora-adapters-dir /path/to/lora_outputs \
    #   --output-dir /path/to/evaluation_results \
